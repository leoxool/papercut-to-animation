...
const ChaosGallery = ({ files, onBack, audioUrl }) => {
    ...
    const sceneryRef = useRef({}); // 存放外部模型引用 (传给导演用)
    ...
    const switchMode = (mode) => {
        ...
        if (mode === 'mv') {
            ...
            // 切换场景可见性
            galleryGroupRef.current.visible = false; // 隐藏白墙画廊
            mvGroupRef.current.visible = true;       // 显示黑色舞台
            
            // 切换暗黑氛围背景
            const darkColor = new THREE.Color(0xffeeff);
            if(sceneRef.current) {
                sceneRef.current.background = darkColor;
                sceneRef.current.fog.color = darkColor;
                sceneRef.current.fog.near = 0.1;//10
                sceneRef.current.fog.far = 150; // 400雾更浓
            }

            // 调整灯光 (戏剧性金色顶光)
            if (keyLightRef.current) {
                keyLightRef.current.intensity = 50; 
                keyLightRef.current.color.setHex(0xffaa00); 
                // 导演会接管位置，这里设个初始值
                keyLightRef.current.position.set(0, 100, 50); 
            }
            ...
            // 恢复柔和白光
            if (keyLightRef.current) {
                keyLightRef.current.intensity = 80; 
                keyLightRef.current.color.setHex(0xffffff); 
                keyLightRef.current.position.set(50, 200, 100); 
            }
    ...
    useEffect(() => {
        ...
        mvGroupRef.current.visible = false; // 初始隐藏 MV 场景
        ...
        spotLight.shadow.blurSamples = 25; // 极致柔和
        spotLight.shadow.radius = 10;      // 扩散半径
        ...
        // 6. Gallery Environment (添加到 galleryGroup)
        ...
        gltfLoader.load('/assets/stage.glb', (gltf) => {
            ...
            model.traverse((child) => {
                if (child.isMesh) {
                    ...
                    // 可选：在这里修改 child.material 获得统一风格
                }
            });


            model.position.set(0, 0, 0); // 初始位置
            ...
            sceneryRef.current.stage = model; // 存引用

            console.log("Stage loaded!", model); // 看控制台有没有这行字

        }, undefined, (e) => console.log('Stage load error', e));
        ...
        gltfLoader.load('/assets/tree.glb', (gltf) => {
            ...
            tree.scale.set(1500, 1500, 1500);
            mvGroupRef.current.add(tree);
            sceneryRef.current.tree = tree; // 存引用
        }, undefined, (e) => console.log('Tree load error', e));
        ...
        const sceneContext = {
            ...
            scenery: sceneryRef.current // ★ 传递引用
        };
