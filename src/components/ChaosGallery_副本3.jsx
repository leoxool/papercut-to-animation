...
const ChaosGallery = ({ files, onBack, audioUrl }) => {
    ...
    const keyLightRef = useRef(null); // 指向 spotLight1
    const audioRef = useRef(null); 
    
    // === 场景分组 Refs ===
    const galleryGroupRef = useRef(new THREE.Group()); // 画廊模式物体 (白墙, 地板)
    const mvGroupRef = useRef(new THREE.Group());      // MV 模式物体 (舞台, 树, 舞台灯)
    ...
    const sceneryRef = useRef({}); // 外部模型引用
    ...
    const switchMode = (mode) => {
        ...
        if (mode === 'mv') {
            ...
            // 切换暗黑背景
            const darkColor = new THREE.Color(0xcccccc);
            if(sceneRef.current) {
            ...
            // 恢复基础照明
            if (lightsRef.current.hemiLight) lightsRef.current.hemiLight.intensity = 2.0;
            if (lightsRef.current.dirLight1) {
                lightsRef.current.dirLight1.intensity = 3.0;
                lightsRef.current.dirLight1.color.setHex(0xffffff);
            }
    ...
    useEffect(() => {
        ...
        // 1. Hemisphere (根场景)
        const hemiLight = new THREE.HemisphereLight(0xffffff, 0x444444, 8,0);
        scene.add(hemiLight);
        lightsRef.current.hemiLight = hemiLight;

        // 2. DirLight 1 (根场景 - 主环境)
        ...
        // 4. SpotLight 1 (MV组 - 舞台主光)
        const spotLight1 = new THREE.SpotLight(0xffffff, 0.0);
        ...
        spotLight1.shadow.radius = 30;
        spotLight1.shadow.bias = -0.0001;
        mvGroupRef.current.add(spotLight1);
        lightsRef.current.spotLight1 = spotLight1;
        keyLightRef.current = spotLight1; // 兼容旧引用

        // 5. SpotLight 2 (MV组 - 副光)
        ...
        renderer.setAnimationLoop(() => {
            ...
            // --- 常规模式逻辑 ---
            if (wallRef.current) {
                const targetWallZ = (mode === 'grid') ? -135 : -500;
                ...
                // 常规模式只操作 dirLight1 的阴影
                if (lightsRef.current.dirLight1 && mode === 'grid') {
                      const blurFactor = THREE.MathUtils.mapLinear(dist, 100, 200, 0, 1);
                      lightsRef.current.dirLight1.shadow.radius = THREE.MathUtils.lerp(4, 20, Math.max(0, Math.min(1, blurFactor)));
                }
            }
            if (floorRef.current) {
