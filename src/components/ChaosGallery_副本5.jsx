...
const ChaosGallery = ({ files, onBack, audioUrl }) => {
    ...
    // === 场景分组 Refs ===
    const galleryGroupRef = useRef(new THREE.Group()); // 存放浏览模式物体 (墙, 地板)
    const mvGroupRef = useRef(new THREE.Group());      // 存放 MV 模式物体 (舞台, 树)
    ...
    const switchMode = (mode) => {
        ...
        if (mode === 'mv') {
            ...
            const darkColor = new THREE.Color(0xcceeff);
            if(sceneRef.current) {
                sceneRef.current.background = darkColor;
                sceneRef.current.fog.color = darkColor;
                sceneRef.current.fog.near = 10;
                sceneRef.current.fog.far = 400;
            }

            // MV 模式：让 SpotLight1 回归舞台位置
            if (lightsRef.current.spotLight1) {
                lightsRef.current.spotLight1.position.set(70, 50, 100); // 初始值，后续会被 Director 接管
            ...
            // 🔥 恢复 SpotLight 到浏览状态 (替代 DirectionalLight)
            if (lightsRef.current.spotLight1) {
                lightsRef.current.spotLight1.visible = true;
                lightsRef.current.spotLight1.position.set(70, 70, 80); // 经典右上角主光
                lightsRef.current.spotLight1.intensity = 100;             // 适合浏览的亮度
                lightsRef.current.spotLight1.color.setHex(0xfff5e6);     // 暖白光
            ...
            // 停用或减弱 DirLight (因为我们主要用 SpotLight 了)
            if (lightsRef.current.dirLight1) {
                lightsRef.current.dirLight1.intensity = 0.5; // 仅作为微弱补光
            }
    ...
    useEffect(() => {
        ...
        // 1. Hemisphere (通用环境光) - 根场景
        const hemiLight = new THREE.HemisphereLight(0xffffff, 0xffffff, 8.0);
        scene.add(hemiLight);
        lightsRef.current.hemiLight = hemiLight;

        // 2. SpotLight 1 (主角) - 根场景 (方便在 Gallery 和 MV 模式间复用)
        const spotLight1 = new THREE.SpotLight(0xffffff, 100); // 默认开启，适合 Gallery
        ...
        scene.add(spotLight1); // 🔥 改为添加到 scene，而不是 mvGroup
        lightsRef.current.spotLight1 = spotLight1;
        keyLightRef.current = spotLight1; 

        // 3. DirLight 1 (辅助) - 根场景
        const dirLight1 = new THREE.DirectionalLight(0xffffff, 0.0); // 调暗，避免干扰 SpotLight
        dirLight1.position.set(100, 100, 50);
        dirLight1.castShadow = false; // 关闭阴影，让 SpotLight 负责
        scene.add(dirLight1);
        lightsRef.current.dirLight1 = dirLight1;

        // --- 以下灯光放入 MV Group (只在 MV 模式亮) ---
        ...
        gltfLoader.load('/assets/stage.glb', (gltf) => {
            if (!isMounted) return;
            ...
            mvGroupRef.current.add(model);
            sceneryRef.current.stage = model;
        }, undefined, (e) => console.log('Stage error', e));
        ...
        gltfLoader.load('/assets/tree.glb', (gltf) => {
            ...
            tree.position.set(0, 450, 100);
            tree.scale.set(150, 150, 150);
            mvGroupRef.current.add(tree);
            sceneryRef.current.tree = tree;
        }, undefined, (e) => console.log('Tree error', e));
        ...
        files.forEach((file, index) => {
            loader.load(file.processedUrl, (texture) => {
                ...
                const material = new THREE.MeshStandardMaterial({
                    ...
                    alphaTest: 0.1, roughness: 0.5, metalness: 0.1, emissive: 0x222222, emissiveIntensity: 0.1
                });
    ...
    // ... (Handle input functions 保持不变) ...
    // 为了代码完整性，这里省略了下面的 handleStart, handleMove 等重复代码，
    // 它们与你上一版完全一致，没有修改。
    
    // (请保留 handleStart, handleMove, handleEnd, handleMouseDown... 等函数)
    // (请保留 return JSX)
    // -------------------------------------------------------------
    // 由于篇幅限制，这里不重复粘贴未修改的 Input 处理和 Return 部分，
    // 请直接复用你上一次提供的代码中的这部分。
    ...
    return (
        <div className="fixed inset-0 z-50 flex flex-col watercolor-paper no-select">
            <audio ref={audioRef} src={audioUrl || DEFAULT_MUSIC} loop crossOrigin="anonymous" />
