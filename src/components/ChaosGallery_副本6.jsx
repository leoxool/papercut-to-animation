...
const KaleidoscopeShader = {
    uniforms: {
        uTime: { value: 0 },
        uTexture: { value: null },
        uTiling: { value: 6.0 },   // 几行几列 (多图并置)
        uSegments: { value: 6.0 }, // 万花筒折叠瓣数
    ...
    fragmentShader: `
        ...
        void main() {
            ...
            vec2 gridID = floor(gridUV); // 获取每个格子的ID
            vec2 uv = fract(gridUV);     // 获取格子内部的UV (0~1)
            ...
            // 4. 随机性与位移 (Random & Flow)
            // 根据格子ID生成随机偏移量，让每个格子的图看起来不一样
            vec2 randomOffset = vec2(random(gridID), random(gridID + 1.0));
            
            // 加上时间流逝，让图案向一个方向位移
            vec2 flow = vec2(uTime * 0.1, uTime * 0.15);

            // 5. 采样纹理
            // fract 确保纹理重复平铺
            vec3 color = texture2D(uTexture, fract(newUv + randomOffset + flow)).rgb;
           // gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0);
            gl_FragColor = vec4(color * 2.0, uOpacity); // 亮度翻倍
        }
    `
};
...
const ChaosGallery = ({ files, onBack, audioUrl }) => {
    ...
    useEffect(() => {
        ...
        // 随机取一张采集的蝴蝶图作为舞台纹理，如果没有则不加载
        const stageTextureUrl = (files && files.length > 0) ? files[0].processedUrl : null;

        if (stageTextureUrl) {
            stageLoader.load(stageTextureUrl, (tex) => {
                ...
                const shaderMat = new THREE.ShaderMaterial({
                    uniforms: {
                        uTime: { value: 0 },
                        uTexture: { value: tex },
                        uTiling: { value: 4.0 },    // 4x4 的网格
                       uSegments: { value: 6.0 },  // 六边形万花筒折叠
                        uRadius: { value: 1.0 },
                        uOpacity: { value: 1.0 }
                    },
