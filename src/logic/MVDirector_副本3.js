...
export class MVDirector {
    ...
    update(t, audioTime) {
        ...
        if (shot) this.executeShot(t, audioTime, shot);
        else this.animateIdle(t);
        
        // 2. 演员 (含背景色控制)
        this.animateActors(audioTime);

        // 3. 动态光效
        if (audioTime > 102.23) this.animateDynamicLights(t, audioTime, shot);
    }
     //★★★ 新增：动态灯光编舞 ★★★
    ...
    // ... (findCurrentShot, onShotStart, executeShot, animateDynamicLights, animateIdle 保持不变) ...
    // 为了节省篇幅，这里省略了这些未修改的方法，请保留你上一个版本中这些方法的代码
    // 只列出有修改的 helper 函数
    ...
    onShotStart(shot) {
        ...
        const targetObj = this.getActorObject(shot.camera?.target);
        if (targetObj) this.lookAtPos.copy(targetObj.position);
    }
    executeShot(t, audioTime, shot) {
        ...
        if (camConfig.type === 'FIXED') {
            if (camConfig.pos) destPos.set(...camConfig.pos); else destPos.copy(this.ctx.camera.position); 
            ...
            const dist = camConfig.distance || 30; const speed = camConfig.speed || 0.5; const height = camConfig.height || 10;
            const angle = localTime * speed;
            const camX = targetPos.x + Math.cos(angle) * dist; const camZ = targetPos.z + Math.sin(angle) * dist;
            destPos.set(camX, targetPos.y + height, camZ); destLookAt.copy(targetPos);
        } else if (camConfig.type === 'FOLLOW') {
            if (targetActor) {
                const dist = camConfig.distance || 40; const height = camConfig.height || 10;
                let forward = new THREE.Vector3(0, 0, 1);
                const velocity = targetPos.clone().sub(this.lookAtPos).normalize();
                if (velocity.lengthSq() > 0.001) forward.copy(velocity);
                destPos = targetPos.clone().sub(forward.multiplyScalar(dist)); destPos.y += height; destLookAt.copy(targetPos);
            }
        }
        if (camConfig.shake) this.applyShake(destPos, t, camConfig.shake);
        if (this.isShotStartFrame && shot.cut) {
            this.ctx.camera.position.copy(destPos); this.ctx.camera.lookAt(destLookAt); this.lookAtPos.copy(destLookAt);
        } else {
            const smooth = (camConfig.type === 'FOLLOW') ? (camConfig.stiffness || 0.05) : 0.1;
            this.ctx.camera.position.lerp(destPos, smooth); this.ctx.camera.lookAt(destLookAt); this.lookAtPos.copy(destLookAt);
        }
    ...
    updateFocus(shot) {
        ...
        if (shot.focus) {
            ...
            if (shot.focus.value !== undefined) u['focus'].value = THREE.MathUtils.lerp(u['focus'].value, shot.focus.value, 0.1);
            else if (shot.focus.target) {
                const targetObj = this.getActorObject(shot.focus.target);
                if (targetObj) { const dist = this.ctx.camera.position.distanceTo(targetObj.position); u['focus'].value = THREE.MathUtils.lerp(u['focus'].value, dist, 0.1); }
            }
        } else { u['aperture'].value = 0.00001; u['focus'].value = 200; }
    }
    animateIdle(t) {
        const r = 140; this.ctx.camera.position.x = Math.sin(t * 0.1) * r; this.ctx.camera.position.z = Math.cos(t * 0.1) * r; this.ctx.camera.position.y = 60; this.ctx.camera.lookAt(0, 0, 0);
    }
    ...
    getActorObject(name) {
        if (!name) return null;
        if (name === 'sceneBackground') return this.ctx.scene; // ★ 新增：返回场景对象
    ...
    // ★ 2. 增强属性应用：支持 Scene 背景变色
    applyProperties(obj, prev, next, alpha) {
        let t = alpha;
        if (next.ease === 'STEP') t = 0;

        // 常规对象属性
        if (prev.pos && next.pos) { const x = THREE.MathUtils.lerp(prev.pos[0], next.pos[0], t); const y = THREE.MathUtils.lerp(prev.pos[1], next.pos[1], t); const z = THREE.MathUtils.lerp(prev.pos[2], next.pos[2], t); obj.position.set(x, y, z); }
        if (prev.rot && next.rot) { const x = THREE.MathUtils.lerp(prev.rot[0], next.rot[0], t); const y = THREE.MathUtils.lerp(prev.rot[1], next.rot[1], t); const z = THREE.MathUtils.lerp(prev.rot[2], next.rot[2], t); obj.rotation.set(x, y, z); }
        if (prev.scale && next.scale) { const x = THREE.MathUtils.lerp(prev.scale[0], next.scale[0], t); const y = THREE.MathUtils.lerp(prev.scale[1], next.scale[1], t); const z = THREE.MathUtils.lerp(prev.scale[2], next.scale[2], t); obj.scale.set(x, y, z); }
        if (prev.intensity !== undefined && next.intensity !== undefined) { obj.intensity = THREE.MathUtils.lerp(prev.intensity, next.intensity, t); }
        if (prev.opacity !== undefined && next.opacity !== undefined) { if (obj.material) { obj.material.opacity = THREE.MathUtils.lerp(prev.opacity, next.opacity, t); if (obj.material.opacity < 1.0) obj.material.transparent = true; } }
        
        // 颜色处理 (兼容 Scene 和 Light/Mesh)
        if (prev.color && next.color) { 
            if (obj.isLight) {
                obj.color.set(prev.color).lerp(this.tempColor.set(next.color), t); 
            } else if (obj.material) {
                obj.material.color.set(prev.color).lerp(this.tempColor.set(next.color), t); 
            } else if (obj.isScene) { // ★ 特殊处理场景
                obj.background.set(prev.color).lerp(this.tempColor.set(next.color), t);
                // 同步雾的颜色，否则背景变了雾没变会很难看
                if (obj.fog) {
                    obj.fog.color.copy(obj.background);
                }
            }
        }
    }

    // ... (applySwarmChoreography, calculateFormationPos, handleButterflyOverride, applyBugProperties 保持不变) ...
    // 请确保保留之前的 Swarm 和 Override 逻辑
    applySwarmChoreography({ prev, next, alpha }, time) {
        ...
        let targetRot = null;
        if (prev.rot && next.rot) { targetRot = new THREE.Euler(THREE.MathUtils.lerp(prev.rot[0], next.rot[0], alpha), THREE.MathUtils.lerp(prev.rot[1], next.rot[1], alpha), THREE.MathUtils.lerp(prev.rot[2], next.rot[2], alpha)); }
        bugs.forEach((b, i) => {
            const targetPos = this.calculateFormationPos(i, bugs.length, prev.formation, currentSpread);
            targetPos.add(center);
            const nx = Math.sin(time * 2 + i) * noiseStrength; const ny = Math.cos(time * 1.5 + i * 0.5) * noiseStrength; const nz = Math.sin(time * 1.2 + i * 0.2) * noiseStrength;
            targetPos.add(new THREE.Vector3(nx, ny, nz));
            b.group.position.lerp(targetPos, 0.05); b.group.scale.setScalar(currentScale);
            ...
            b.leftWing.material.opacity = currentOpacity; b.leftWing.material.color.setHex(0xffffff);
        });
    }
    calculateFormationPos(index, total, type, spread) {
        const pos = new THREE.Vector3();
        if (type === 'GRID') { const cols = Math.ceil(Math.sqrt(total)); const row = Math.floor(index / cols); const col = index % cols; const spacing = 15 * spread; pos.set((col - cols/2) * spacing, (row - cols/2) * spacing, 0); } 
        else if (type === 'SPHERE') { const phi = Math.acos(-1 + (2 * index) / total); const theta = Math.sqrt(total * Math.PI) * phi; const r = 40 * spread; pos.set(r * Math.cos(theta) * Math.sin(phi), r * Math.sin(theta) * Math.sin(phi), r * Math.cos(phi)); } 
        else if (type === 'VORTEX') { const angle = index * 0.5; const radius = 10 + index * 0.5 * spread; const height = index * 2 * spread - (total); pos.set(Math.cos(angle) * radius, height, Math.sin(angle) * radius); }
        return pos;
    }
    handleButterflyOverride(actorName, keyframes, currentTime) {
        ...
        if (!frameData) return;
        let targetIndices = [];
        if (actorName.startsWith('bugs_')) { const parts = actorName.split('_'); const start = parseInt(parts[1]); const end = parseInt(parts[2]); for (let i = start; i <= end; i++) targetIndices.push(i); } else { targetIndices.push(parseInt(actorName.split('_')[1])); }
        targetIndices.forEach(idx => { const bug = this.ctx.butterflies[idx]; if (bug) this.applyBugProperties(bug, frameData.prev, frameData.next, frameData.alpha, currentTime); });
    }
    applyBugProperties(bug, prev, next, alpha, time) {
        let t = alpha; if (next.ease === 'STEP') t = 0;
